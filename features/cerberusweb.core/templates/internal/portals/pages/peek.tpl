...
{$is_writeable = Context_PortalPage::isWriteableByActor($dict, $active_worker)}
...
$(function() {
	...
	$popup.one('popup_open',function(event,ui) {
		$popup.dialog('option','title', "{'portal_page'|devblocks_translate|capitalize|escape:'javascript' nofilter}");
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=portal_page&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank', 'noopener');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=portal_page&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});
