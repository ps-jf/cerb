{$view_context = 'cerb.contexts.portal.page'}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=portal_page&id={$result.p_id}-{$result.p_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.p_name}</a>
			...
			{elseif $column == "p_portal_id"}
			<td>
				{$portal = $portals.{$result.$column}}
				{if $portal}
					<a href="javascript:;" class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_PORTAL}" data-context-id="{$portal->id}">{$portal->name}</a>
				{else}
					{$result.$column}
				{/if}
			</td>
			...
			<td>
				{$display_ext = Extension_PortalPage::get($result.$column)}
				{if $display_ext}
