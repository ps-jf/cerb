{$view_context = 'cerb.contexts.portal.widget'}
	...
	<tbody style="cursor:pointer;">
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td>
				...
				<a href="{devblocks_url}c=profiles&type=portal_widget&id={$result.p_id}-{$result.p_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.p_name}</a>
			...
			<td>
				{$display_ext = Extension_PortalWidget::get($result.$column)}
				{if $display_ext}
			...
			{elseif $column == "p_portal_page_id"}
			<td>
				{$portal_page = $portal_pages.{$result.$column}}
				{if $portal_page}
					<a href="javascript:;" class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_PORTAL_PAGE}" data-context-id="{$portal_page->id}">{$portal_page->name}</a>
				{else}
					{$result.$column}
				{/if}
			</td>
