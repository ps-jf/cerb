...
$(function() {
	...
	$toolbar.find('button.tester').click(function(e) {
		...
		var $widget_params = $field.closest('.cerb-extension-params');
		...
		$widget_params.find('[name="' + field_key + '"]')
			.each(function(index) { 
				...
				if($this.is($field)) {
					genericAjaxPost($div.closest('form'), divTester, 'c=profiles&a=handleSectionAction&section=portal_widget&action=testWidgetTemplate&template_key=' + encodeURIComponent(field_key) + '&index=' + index);
				}
			}
		);
	});
