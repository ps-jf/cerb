{$view_context = CerberusContexts::CONTEXT_IDENTITY}
...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="worklistBody">
	...
	{$object_addys = []}
	{if in_array(SearchFields_Identity::EMAIL_ID, $view->view_columns)}
		{$addy_ids = array_unique(DevblocksPlatform::extractArrayValues($results, 'i_email_id'))}
		{$object_addys = DAO_Address::getIds($addy_ids)}
	{/if}
	
	{* Bulk lazy identity pools *}
	{$object_pools = []}
	{if in_array(SearchFields_Identity::POOL_ID, $view->view_columns)}
		{$pool_ids = array_unique(DevblocksPlatform::extractArrayValues($results, 'i_pool_id'))}
		{$object_pools = DAO_IdentityPool::getIds($pool_ids)}
	{/if}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			<td data-column="*_watchers" align="center" nowrap="nowrap" style="padding:5px;">
				{include file="devblocks:cerberusweb.core::internal/watchers/context_follow_button.tpl" context=$view_context context_id=$result.i_id}
			</td>
		{foreach from=$view->view_columns item=column name=columns}
			{if substr($column,0,3)=="cf_"}
				{include file="devblocks:cerberusweb.core::internal/custom_fields/view/cell_renderer.tpl"}
			{elseif $column == "i_name"}
			<td>
				<input type="checkbox" name="row_id[]" value="{$result.i_id}" style="display:none;">
				<a href="{devblocks_url}c=profiles&type=identity&id={$result.i_id}-{$result.i_name|devblocks_permalink}{/devblocks_url}" class="subject">{$result.i_name}</a>
				<button type="button" class="peek cerb-peek-trigger" data-context="{$view_context}" data-context-id="{$result.i_id}"><span class="glyphicons glyphicons-new-window-alt"></span></button>
			</td>
			{elseif in_array($column,["i_pool_id"])}
				{$pool = $object_pools.{$result.$column}}
				<td data-column="{$column}">
					{if $pool}
					<a href="javascript:;" class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_IDENTITY_POOL}" data-context-id="{$result.$column}">
						{$pool->name}
					</a>
					{/if}
				</td>
			{elseif in_array($column,["i_email_id"])}
				{$email = $object_addys.{$result.$column}}
				<td data-column="{$column}">
					{if $email}
						{$email_label = $email->email}
					{/if}
					<a href="javascript:;" class="cerb-peek-trigger no-underline" data-context="{CerberusContexts::CONTEXT_ADDRESS}" data-context-id="{$result.$column}" title="{$email_label}">
						{$email_label|truncate:45:'...':true:true}
					</a>
				</td>
			{elseif in_array($column, ["i_email_verified","i_phone_number_verified"])}
				<td>
			...
			{elseif in_array($column, ["i_created_at","i_updated_at"])}
				<td>
					{if !empty($result.$column)}
