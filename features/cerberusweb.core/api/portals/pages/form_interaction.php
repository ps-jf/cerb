<?php
class PortalPage_FormInteraction extends Extension_PortalPage {
	const ID = 'cerb.portal.page.form_interaction';
	
	public function renderConfig(Model_PortalPage $model) {
		$tpl = DevblocksPlatform::services()->template();
		
		$tpl->assign('model', $model);
		$tpl->display('devblocks:cerberusweb.core::portals/builder/pages/form_interaction/config.tpl');
	}
	...
	function render(Model_PortalPage $page, Model_CommunityTool $portal, DevblocksHttpResponse $response) {
		$renderer = new Extension_PortalPageRenderer(function() use ($page, $portal, $response) {
			...
			$tpl->display('devblocks:cerberusweb.core::portals/builder/pages/form_interaction.tpl');
		});
	...
	function renderForm(Model_PortalPage $page, Model_CommunityTool $portal) {
		...
		if($reset || null == ($state_id = $session->getProperty($state_key, null))) {
			$interaction = $this->_startFormSession($page);
		...
		$this->_renderFormState($page, $interaction);
	}
	
	private function _startFormSession(Model_PortalPage $page) {
		$session = ChPortalHelper::getSession();
		$portal = ChPortalHelper::getPortal();
		...
		if(false == ($interaction_behavior = DAO_TriggerEvent::get($behavior_id)))
			return;
		...
		@$behavior_vars = DevblocksPlatform::importVar(@$page->params['behavior_vars'], 'array', []);
	...
	private function _renderFormState(Model_PortalPage $page, Model_BotSession $interaction) {
		$tpl = DevblocksPlatform::services()->template();
		$identity = ChPortalHelper::getIdentity();
		...
		if($resume_path) {
			// Did we even try to submit?
			if('POST' == DevblocksPlatform::strUpper(DevblocksPlatform::getHttpMethod())) {
				$this->_prepareResumeDecisionTree($behavior, $prompts, $interaction, $actions, $dict, $resume_path);
